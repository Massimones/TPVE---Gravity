function [x,e11,e22,e33,u1,u3,e111ve,e221ve,e331ve,ux1ve,uz1ve,e112ve,e222ve,e332ve,ux2ve,uz2ve,tau11ve,tau22ve,tau33ve,tau111ve,tau221ve,tau331ve,tau112ve,tau222ve,tau332ve,e13,e131ve,e132ve,tau13ve,tau131ve,tau132ve]=TPE_SOLUTIONS_TOTAL(H,alfa,dp,dT,a,db,ni,mu,lambda,MedianPlane,limiteplot,zlm,k,eta,TV,TV2)
...
% dp=10e6;                  % pore pressure change of disk 1
% dT=100;                  % Temperature change of disk 1
% % dpB=1e6;               % pore pressure change of disk 2
% % dTB=100;                   % Temperature change of disk 1
% a=500;                    % disk radius
% db=140;                  % disk height 
...
% limiteplot=10000;         % Limit in plot (max(x))
...
%  e11s=errE.*(st.^2).*(cp.^2)+2.*ertE.*st.*ct.*(cp.^2)+ettE.*(ct.^2).*(cp.^2)+effE.*(sp.^2);
%  e22s=errE.*(st.^2).*(sp.^2)+2.*ertE.*st.*ct.*(sp.^2)+ettE.*(ct.^2).*(sp.^2)+effE.*(cp.^2);
%  e33s=errE.*(ct.^2)-2*ertE.*st.*ct+ettE.*(st.^2);
...
% plot(x,-uz1ve,'--');
% plot(x,-uz2ve,'--');
% legend(['+u_1 t/\tau= 0'],['+u_1 t/\tau=',num2str(ttau)],['+u_1 t/\tau=',num2str(ttau2)],['- u_3 t/\tau= 0'],['- u_3 t/\tau=',num2str(ttau)],['- u_3 t/\tau=',num2str(ttau2)]);
% title('Visco-Elastic Displacement (m)');
% xlabel(' x (m)');
% ylabel(' u_1 ({\bf x},t), -u_3 ({\bf x},t)');
% set(gca,'FontSize',14);
% 
% 
% figure()
% plot(x,tau11,x,tau22,x,tau33);
% hold on
% %plot(x,tau11s,x,tau22s,x,tau33s);
% 
% legend('tau11','tau22','tau33');
% title('Elastic \tau ij');
% xlabel(' x (m)');
% ylabel(' \tau ij');
% %ylim([-8e7 8e7]);
...
% 
% figure()
% plot(x,e11,x,e111ve,x,e112ve,x,e22,x,e221ve,x,e222ve,x,e33,x,e331ve,x,e332ve);
% hold on
% %plot(x,tau11s,x,tau22s,x,tau33s);
% 
% legend(['e11 t/\tau= 0'],['e11 t/\tau=',num2str(ttau)],['+e11 t/\tau=',num2str(ttau2)],['e22 t/\tau= 0'],['e22 t/\tau=',num2str(ttau)],['e22 t/\tau=',num2str(ttau2)],['e33 t/\tau= 0'],['e33 t/\tau=',num2str(ttau)],['e33 t/\tau=',num2str(ttau2)]);
% 
% title('Elastic e_{ij}');
% xlabel(' x (m)');
% ylabel(' e_{ij} ');
...
% plot(x,ux1ve);
% plot(x,ux2ve);
% legend(['t/\tau= 0'],['t/\tau=',num2str(ttau)],['t/\tau=',num2str(ttau2)]);
% title('Visco-Elastic u_x');
% xlabel(' x (m)');
% ylabel(' u_x (m)');
% set(gca,'FontSize',14);
% 
